...
export default observer(function Qwerty({ store }: QwertyProps) {
    ...
    return (
        <div>
            {qwerty.map((row) => (
                <div className="flex justify-center gap-x-1 mt-1" key={row}>
                    {row.split('').map((key, index) => {
                        if (key === 'D' && row.startsWith('Delete')) {
                            return (
                                <button
                                    className="cursor-pointer m-px px-1 text-black flex h-10 w-20 items-center justify-center rounded-md bg-gray-300 uppercase"
                                    key="delete"
                                    onClick={() =>
                        ...
                        return (
                            <button
                                className={`cursor-pointer m-px text-black flex h-10 w-10 items-center justify-center rounded-md uppercase ${bgColor}`}
                                key={key}
